 ...
 */
private fun schemeFromSeed(seed: Color, darkTheme: Boolean): ColorScheme {
    ...
    return if (!darkTheme) {
        lightColorScheme(
            ...
            tertiary = mix(seed, black, 0.3f),
            onTertiary = white,
            tertiaryContainer = mix(seed, white, 0.75f),
            onTertiaryContainer = mix(seed, black, 0.55f),
            background = md_theme_light_background,
            onBackground = md_theme_light_onBackground,
            surface = md_theme_light_surface,
            onSurface = md_theme_light_onSurface,
        ...
        darkColorScheme(
            primary = mix(seed, white, 0.45f),
            onPrimary = mix(seed, black, 0.7f),
            primaryContainer = mix(seed, black, 0.35f),
            onPrimaryContainer = mix(seed, white, 0.85f),
            secondary = mix(seed, white, 0.35f),
            onSecondary = mix(seed, black, 0.7f),
            secondaryContainer = mix(seed, black, 0.45f),
            onSecondaryContainer = mix(seed, white, 0.8f),
            tertiary = mix(seed, white, 0.55f),
            onTertiary = mix(seed, black, 0.75f),
            tertiaryContainer = mix(seed, black, 0.4f),
            onTertiaryContainer = mix(seed, white, 0.85f),
            background = md_theme_dark_background,
            onBackground = md_theme_dark_onBackground,
            surface = md_theme_dark_surface,
            onSurface = md_theme_dark_onSurface,
...
fun WatchVaultTheme(
    ...
    val colorScheme = when {
        ...
        else -> schemeFromSeed(Color(seedColor.argb), darkTheme)
    }
